 ...
 */
object SimilarityAnalysisAndryw extends Serializable {
  ...
  def cooccurrences(drmARaw: DrmLike[Int], randomSeed: Int = 0xdeadbeef, maxInterestingItemsPerThing: Int = 50,
    ...
    //    "elementDelim" -> " ",
    //      "omitScore" -> false)
    //    println(drmAtA.collect)

    //    drmAtA.dfsWrite("tmp/drmAtA.csv")
    //    println(drmAtA.ncol)
    //    println(drmAtA.nrow)

    //    val drmAtAA = drmA.t %*% drmA
    //    println(drmAtAA.ncol)
    //    println(drmAtAA.nrow)

    //    val andryw = drmA  %*% drmATA
    //    andryw.dfsWrite("tmp/andryw.csv",writeSchema)
    //    val drmAtAAA = drmA.t %*% drmA
    //    drmAtAAA.dfsWrite("tmp/drmAtAAA.csv",writeSchema)
  ...
  def cooccurrencesAndryw(drmARaw: DrmLike[Int], randomSeed: Int = 0xdeadbeef, maxInterestingItemsPerThing: Int = 50,
                          maxNumInteractions: Int = 500, drmBs: Array[DrmLike[Int]] = Array()): (DrmLike[Int],DrmLike[Int]) = {
    ...
    val ola = drmA %*% drmAtA
    println(ola.collect)
    val ola1 = drmA %*% drmSimilarityAtA
    println(ola1.collect)
